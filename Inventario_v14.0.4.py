...
class App:
    def __init__(self):
        ...
        frame0 = customtkinter.CTkFrame(self.app)
        frame0.pack(padx=0, pady=0)

        self.info_label3 = customtkinter.CTkLabel(frame0, text="", justify="center", font=("Helvetica", 12, "bold"))
        ...
        #FRAME UNO DATOS OBTENIDOS y BOTON
    ...
    def mostrar_informacion(self):
        ...
        if resultado_bd:
            ...
            info_text3 += f"UNIDAD: Ref.{self.id_unidad}"
            info_text3 += f" - {self.nombre_unidad}"
            self.info_label3.configure(text=info_text3)
        else:
            #messagebox.showinfo("Información", "No se encontró información en la base de datos para esta dirección IP.")
    ...
    def guardar_informacion(self):
        try:
            ...
            cursor = conn.cursor()
            if not db_exists:
                cursor.execute('''CREATE TABLE InventarioCDI (
                ...
                cursor.execute('''INSERT INTO InventarioCDI 
                               (numero_serial, 
                               id_unidad, 
                               nombre_unidad, 
                               nombre_equipo, 
