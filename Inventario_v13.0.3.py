...
class Aplicacion(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        self.estilo = set_appearance_mode("#2b2b2b")
    ...
    def crear_widgets(self):
        ...
        self.grid = ttk.LabelFrame(self, text="Datos Obtenidos Para el Inventario", labelanchor="n", style="Rounded.TLabelframe") #Labelanchor n para centrar texto North se usa NSWE norte sur este oeste
        ...
        for i, etiqueta in enumerate(self.etiquetas, start=1):
            etiqueta_widget = ttk.Label(self.grid, text=etiqueta, style="Rounded.Label")
            etiqueta_widget.grid(row=i, column=0, sticky="w", padx=5, pady=5)

            dato_widget = ttk.Label(self.grid, text="", style="Rounded.Label")
            dato_widget.grid(row=i, column=1, sticky="w", padx=5, pady=5)
            self.etiquetas_datos[etiqueta] = dato_widget

        boton_enviar = ttk.Button(self, text="Enviar Datos", command=self.enviar_datos, style="DarkBlue.TButton")
